...
function Api() {
  ...
  const buscarResultados = async () => {
    const API_KEY = 'P3aLBB27lAVINn37PNQldpx5L8ajMGFm4_kchZH1Eww';
    const URL = `https://api.unsplash.com/search/photos/?client_id=${API_KEY}&query=${valor} city&per_page&orientation=landscape`;
    ...
    <div className="App">
      <div className="search__box">
        <input
          ...
          placeholder="Buscar imágenes de ciudades"
          onChange={(e) => setValor(e.target.value)}
        />
        <button onClick={() => buscarResultados()} className="search__box--btn">
          Buscar
        </button>
      </div>
      <div className="main__content">
        <div className="main__content--grid">
          {resultados.map((elemento, indice) => {
            return <img key={indice} src={elemento.urls.regular} alt={`City-${indice}`} />;
          })}
        </div>
      </div>
    </div>
